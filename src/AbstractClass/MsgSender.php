...
use MicroService\ConfigClass\MsgSenderConfig;
 ...
 */
abstract class MsgSender implements MicroServiceCall
{
    protected $driver;
    protected $instance;
    protected $config;
    ...
    /**
     * 检测参数
     * @param MsgSenderConfig $config
     * @throws Exception
     */
    protected function check_params(MsgSenderConfig $config)
    {
        ...
        if (empty($receiver) || empty($template_id)) {
            throw new Exception("receiver & template_id can not be empty!\nCall administrator");
        }
        ...
        if (!is_string($receiver) || !is_string($template_id) || !is_string($channel) || !is_string($account) || !is_string($type) ) {
